...
public class TicketController {
    ...
    public Ticket update(@PathVariable String id, @RequestBody Ticket updated) {
        return repository.findById(id).map(old -> {
            ...
            if (responseAdded) {
                if (old.getRespondedAt() == null) {
                    updated.setRespondedAt(Instant.now().toString());
                }
                notificationRepository.save(Notification.create(
                    updated.getLecturer(),
                ...
                updated.setRespondedAt(old.getRespondedAt());
            }
            
            if ("RESOLVED".equalsIgnoreCase(updated.getStatus()) && !"RESOLVED".equalsIgnoreCase(old.getStatus())) {
                updated.setResolvedAt(Instant.now().toString());
            } else if (!"RESOLVED".equalsIgnoreCase(updated.getStatus())) {
                updated.setResolvedAt(null);
            } else {
                updated.setResolvedAt(old.getResolvedAt());
            }
